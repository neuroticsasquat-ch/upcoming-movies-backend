...
class UsageFakeClient:
    """Returns a fixed Usage per call, routing decisions on the same prompt sniff the
    production FakeClient uses (entity-linking classifier vs cluster)."""
...
async def test_run_link_ingest_records_link_and_cluster_usage(session):
    ...
    assert rows["cluster"].batched is False
