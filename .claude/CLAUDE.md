...
- **The prod compose fallbacks are pinned to the code defaults by test.** `docker-compose.prod.yml` passes retrieval's constants through as `${NAME:-default}`, and production reads its environment from that file — so a fallback left stale there *overrides* a retuned code default rather than merely disagreeing with it. `test_prod_compose_fallbacks_match_the_code_defaults` enforces the pair. It reads the file through a single-file bind mount, which tracks the inode: after an editor that writes-then-renames, the container reads the pre-edit copy and the test can pass on stale content. CI reads the checkout directly and is authoritative; `docker compose up -d --force-recreate upmovies-backend` resyncs locally.
